...
var videoLinksFile = "WWDC2013_links.txt";
var subtitlesFolderForHD = "subtitles/HD/";
var subtitlesFolderForSD = "subtitles/SD/";

var videoURLRegex = /(http:\/\/devstreaming.apple.com\/videos\/wwdc\/2013\/\w+\/\d+\/)(\d+\-\w+)\.mov\?dl=1/;
async.waterfall([
    //read links file
    function(callback) {
        fs.readFile(videoLinksFile, 'utf-8', function (err, data) {
            ...
            var links = lines.filter(function (line) {
                return line && line.length > 0 && line.indexOf('.mov') !== -1;
            });
            callback(null, links);
        });
    },
          ...
          // console.log(url);
            ...
            if (group) {
                return {
                    ...
                    subtitleNameForSD: subtitlesFolderForSD + group[2].replace("-HD", "-SD") + ".srt",
                    webvttFileNames: [],
                    errorMessage: null,
                    skip:false
                };
