...
*/
class DailyTradeInfoSource: NSObject {

	static var data: Array<Any>? = nil

	/*
	* return the static raw data, just strings.
	*/
	static var rawData: Array<String> = {
	...
	*/
	static var dailyTradeInfos: Array<DailyTradeInfo> = {
		let parsingLocale = Locale(identifier: "en_US")
		...
		let raw = DailyTradeInfoSource.rawData
		...
		while i<count {
			...
			//Date           Open       High       Low        Close      Volume        Adj Close
			let stock = DailyTradeInfo()
			let stringDate = raw[i]
			stock.tradingDate = dateFormatter.date(from: stringDate)!
			stock.openingPrice = priceFormatter.number(from: raw[i + 1])
			stock.highPrice = priceFormatter.number(from: raw[i + 2])
			stock.lowPrice = priceFormatter.number(from: raw[i + 3])
			stock.closingPrice = priceFormatter.number(from: raw[i + 4])
			let volumeString = raw[i + 5]
			stock.tradingVolume = priceFormatter.number(from: volumeString)!
			stockPrices.append(stock)
			
			i += 7
		}
		
		let data: [DailyTradeInfo] = stockPrices.sorted { $0.tradingDate < $1.tradingDate }
		return data
	}()
