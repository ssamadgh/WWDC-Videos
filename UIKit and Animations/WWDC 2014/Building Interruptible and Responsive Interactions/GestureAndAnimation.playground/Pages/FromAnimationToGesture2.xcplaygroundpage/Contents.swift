...
//		let view3 = self.configureView(at: position3)
		...
		dynamicItemBehavior = UIDynamicItemBehavior(items: [view])
//		dynamicItemBehavior.resistance = 3.0
//		dynamicItemBehavior.angularResistance = 3.0
		
		collisionBehavior = UICollisionBehavior(items: [view])
		collisionBehavior.translatesReferenceBoundsIntoBoundary = true
		
		dynamicAnimator?.addBehavior(dynamicItemBehavior)
		dynamicAnimator?.addBehavior(collisionBehavior)
		
		dynamicItemBehavior.addLinearVelocity(CGPoint(x: 100, y: 50), for: view)
