...
class AuthStore {
    ...
    func logout() {
		SSKeychain.deletePassword(forService: AuthStore.KeychainServiceName, account: AuthStore.KeychainAccount)
    }
    
    func savedAuthToken() -> String? {
		return SSKeychain.password(forService: AuthStore.KeychainServiceName, account: AuthStore.KeychainAccount)
    }
