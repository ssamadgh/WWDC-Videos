...
class APLTableViewController: UITableViewController, MFMailComposeViewControllerDelegate, SectionHeaderViewDelegate {
	...
	var plays: Array<APLPlay> = [] {
		didSet {
			if !plays.isEmpty {
				self.alphabeticPlays = plays.sorted { $0.name < $1.name }
			}
		}
	}
	...
	var sectionInfoArray: [APLSectionInfo]! {
		didSet {
			if !self.sectionInfoArray.isEmpty {
				self.alphabeticSectionInfoArray = self.sectionInfoArray.sorted { $0.play.name < $1.play.name }
			}
		}
	}
	
	var alphabeticSectionInfoArray: [APLSectionInfo]!
	...
	var alphabetized: Bool = false
	var alphabeticPlays: [APLPlay] = []
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
		...
		return self.alphabetized ? self.alphabeticPlays.count : self.plays.count
    }

    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        // #warning Incomplete implementation, return the number of rows
		let sectionInfo = self.alphabetized ? self.alphabeticSectionInfoArray[section] : self.sectionInfoArray[section]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let sectionInfo = self.alphabetized ? self.alphabeticSectionInfoArray[indexPath.section] : self.sectionInfoArray[indexPath.section]
		let play = sectionInfo.play
	...
	@IBAction func toggleSort(_ sender: UIBarButtonItem) {
		var currentArray: [APLPlay] = []
		var newArray: [APLPlay] = []
		
		if self.alphabetized {
			currentArray = self.alphabeticPlays
			newArray = self.plays
		}
		else {
			currentArray = self.plays
			newArray = self.alphabeticPlays
		}
		
		self.alphabetized = !self.alphabetized
		
		self.tableView.beginUpdates()
		
		for (currentIndex, currentPlay) in currentArray.enumerated() {
			
			newEnumeration: for (newIndex, newPlay) in newArray.enumerated() {
				if newPlay == currentPlay {
					self.tableView.moveSection(currentIndex, toSection: newIndex)
					break newEnumeration
				}
			}
		}
